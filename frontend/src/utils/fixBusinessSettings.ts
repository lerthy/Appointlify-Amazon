...
export const fixBusinessSettings = async () => {
  try {
    ...
    if (usersWithoutSettings.length === 0) {
      console.log('All users already have business settings');
      return;
    }
    
    console.log(`Found ${usersWithoutSettings.length} users without business settings`);
    ...
    console.log(`Successfully created business settings for ${insertedSettings?.length || 0} users`);
    ...
    if (usersWithoutServices.length > 0) {
      console.log(`Found ${usersWithoutServices.length} users without services`);
      ...
      if (insertServicesError) {
        ...
        console.log(`Successfully created services for ${insertedServices?.length || 0} users`);
      }
    }
    ...
    if (usersWithoutEmployees.length > 0) {
      console.log(`Found ${usersWithoutEmployees.length} users without employees`);
      ...
      if (insertEmployeesError) {
        ...
        console.log(`Successfully created employees for ${insertedEmployees?.length || 0} users`);
      }
    }
    
    console.log('Business settings, services, and employees fix completed successfully!');
